...
open class Context {
	...
	public init(dictionary: [String: Any?]) {
		variables = dictionary.mapValues({ Context.parseAny($0) }).filter({ $0.value != nil }).mapValues { $0! }
	}
...
private extension Dictionary where Key == String, Value == Token.Value {
	...
	func valueFor(keyPath: String) -> Token.Value? {
		...
		case (let .array(array), let .some(index), nil) where (0 ..< array.count).contains(index):
			return array[index]

		case let (.array(array), .some(index), .some(remainder)) where (0 ..< array.count).contains(index):
			guard case let .dictionary(dictionary) = array[index] else {
				return nil
			}
		...
		case (let .array(array), nil, let .some(remainder)) where remainder.hasPrefix("first"):
			...
			if rangeFirst.upperBound == remainder.endIndex {
				return array.first
			} else if case let .some(.dictionary(dictionary)) = array.first {
				return dictionary.valueFor(keyPath: String(remainder[rangeFirst.upperBound...]))
			} else {
				return nil
			}

		case (let .array(array), nil, let .some(remainder)) where remainder.hasPrefix("last"):
			let rangeLast = remainder.range(of: "last\\.?", options: .regularExpression)!

			if rangeLast.upperBound == remainder.endIndex {
				return array.last
			} else if case let .some(.dictionary(dictionary)) = array.last {
		...
		case (let .dictionary(dictionary), nil, let .some(remainder)):
			return dictionary.valueFor(keyPath: remainder)
...
private class SupplementalContext: Context {
	...
	init(supplemented: Context, variables: [String: Token.Value]) {
		supplementedContext = supplemented

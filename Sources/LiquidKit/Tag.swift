...
open class Tag {
	...
	open func parse(statement: String, using parser: Parser, currentScope _: Parser.Scope) throws {
	...
	public enum Errors: Error {
		...
		var localizedDescription: String {
			switch self {
			case let .malformedStatement(description): return description
...
class TagElse: Tag {
	...
	override func didDefine(scope: Parser.Scope, parser: Parser) {
		...
		if scope.parentScope?.tag is TagCase {
			...
			// We need to check if this else tag isn't part of another if/elsif/when clause, since it can be structurally identical to a for-else clause.
...
class TagUnless: TagIf {
	...
	override func didDefine(scope: Parser.Scope, parser: Parser) {
		...
		if let conditional = (compiledExpression["conditional"] as? Token.Value) {
			scope.outputState = conditional.isTruthy ? .disabled : .enabled
		}
	}
}
...
class TagCase: Tag {
	...
	override func didDefine(scope: Parser.Scope, parser _: Parser) {
...
class TagEndCase: Tag {
	...
	override func didTerminate(scope: Parser.Scope, parser _: Parser) {
...
class TagCycle: Tag {
	private var groupIdentifier: String? {
		guard case let .some(.string(groupIdentifier)) = compiledExpression["group"] as? Token.Value else {
			return nil
		}
	...
	override func parse(statement: String, using parser: Parser, currentScope: Parser.Scope) throws {
		...
		guard case let .some(.array(groupElements)) = compiledExpression["list"] as? Token.Value else {
			throw Errors.missingArtifacts
		}
...
class TagTablerow: TagFor {
	internal private(set) var tableRowIterator: TableRowIterator!

...
extension Filter {
	...
	static let ceil = Filter(identifier: "ceil") { (input, _) -> Token.Value in
	...
	static let compact = Filter(identifier: "compact") { (input, _) -> Token.Value in
		guard case let .array(inputArray) = input else {
			return .nil
		}
	...
	static let concat = Filter(identifier: "concat") { (input, parameters) -> Token.Value in
		guard case let .array(inputArray) = input, case let .some(.array(parameterArray)) = parameters.first else {
			return .nil
		}
	...
	static let date = Filter(identifier: "date") { (input, parameters) -> Token.Value in
		...
		strFormatter.format = formatString

		return .string(strFormatter.string(from: date!))
	}
	...
	static let dividedBy = Filter(identifier: "divided_by") { (input, parameters) -> Token.Value in
		guard let dividendDouble = input.doubleValue, let divisor = parameters.first else {
			return .nil
		}

		switch divisor {
		case let .integer(divisorInt):
	...
	static let downcase = Filter(identifier: "downcase") { (input, _) -> Token.Value in
		guard case let .string(inputString) = input else {
			return .nil
		}
	...
	static let escape = Filter(identifier: "escape") { (input, _) -> Token.Value in
		return .string(input.stringValue.htmlEscape(decimal: true, useNamedReferences: true))
	}

	static let escapeOnce = Filter(identifier: "escape_once") { (input, _) -> Token.Value in
		return .string(input.stringValue.htmlUnescape().htmlEscape(decimal: true, useNamedReferences: true))
	}

	static let first = Filter(identifier: "first") { (input, _) -> Token.Value in
		guard case let .array(inputArray) = input else {
			return .nil
		}
	...
	static let floor = Filter(identifier: "floor") { (input, _) -> Token.Value in
		guard let inputDouble = input.doubleValue else {
			return .nil
		}
	...
	static let join = Filter(identifier: "join") { (input, parameters) -> Token.Value in
		guard
			let firstParameter = parameters.first,
			case let .string(glue) = firstParameter,
			case let .array(inputArray) = input
		else {
			return .nil
		}
	...
	static let last = Filter(identifier: "last") { (input, _) -> Token.Value in
		guard case let .array(inputArray) = input else {
			return .nil
		}
	...
	static let leftStrip = Filter(identifier: "lstrip") { (input, _) -> Token.Value in
		guard case let .string(inputString) = input else {
			return .nil
		}

		let charset = CharacterSet.whitespacesAndNewlines
		let firstNonBlankIndex = inputString.firstIndex { char -> Bool in
	...
	static let map = Filter(identifier: "map") { (input, parameters) -> Token.Value in
		guard case let .array(inputArray) = input, let keyName = parameters.first?.stringValue else {
			return .nil
		}

		return .array(inputArray.compactMap({
			guard case let .dictionary(dictionaryValue) = $0 else {
				return nil
			}
	...
	static let minus = Filter(identifier: "minus") { (input, parameters) -> Token.Value in
	...
	static let modulo = Filter(identifier: "modulo") { (input, parameters) -> Token.Value in
	...
	static let newlineToBr = Filter(identifier: "newline_to_br") { (input, _) -> Token.Value in
		guard case let .string(inputString) = input else {
			return .nil
		}
	...
	static let plus = Filter(identifier: "plus") { (input, parameters) -> Token.Value in
		guard let decimalInput = input.decimalValue, let decimalParameter = parameters.first?.decimalValue else {
			return .nil
		}
	...
	static let prepend = Filter(identifier: "prepend") { (input, parameters) -> Token.Value in
		guard let stringParameter = parameters.first?.stringValue else {
			return .nil
		}
	...
	static let remove = Filter(identifier: "remove") { (input, parameters) -> Token.Value in
	...
	static let removeFirst = Filter(identifier: "remove_first") { (input, parameters) -> Token.Value in
		guard let needle = parameters.first?.stringValue else {
			return .nil
		}
	...
	static let replace = Filter(identifier: "replace") { (input, parameters) -> Token.Value in
	...
	static let replaceFirst = Filter(identifier: "replace_first") { (input, parameters) -> Token.Value in
		guard parameters.count == 2 else {
			return .nil
		}
	...
	static let reverse = Filter(identifier: "reverse") { (input, _) -> Token.Value in
		guard case let .array(inputArray) = input else {
			return .nil
		}
	...
	static let round = Filter(identifier: "round") { (input, parameters) -> Token.Value in
		guard let inputDouble = input.doubleValue else {
			return .nil
		}
	...
	static let rightStrip = Filter(identifier: "rstrip") { (input, _) -> Token.Value in
		guard case let .string(inputString) = input else {
			return .nil
		}

		let charset = CharacterSet.whitespacesAndNewlines
		let lastNonBlankIndex = inputString.firstIndex(reverse: true) { char -> Bool in
			guard char.unicodeScalars.count == 1, let unichar = char.unicodeScalars.first else {
				return true
			}
	...
	static let size = Filter(identifier: "size") { (input, _) -> Token.Value in
		switch input {
		case let .string(string): return .integer(string.count)
		case let .array(array): return .integer(array.count)
		default:
			return .nil
		}
	}

	static let slice = Filter(identifier: "slice") { (input, parameters) -> Token.Value in
		guard
			case let .string(stringInput) = input,
	...
	static let sort = Filter(identifier: "sort") { (input, _) -> Token.Value in
		guard case let .array(arrayInput) = input else {
			return .nil
		}
	...
	static let sortNatural = Filter(identifier: "sort_natural") { (input, _) -> Token.Value in
		guard case let .array(arrayInput) = input else {
			return .nil
		}
	...
	static let split = Filter(identifier: "split") { (input, parameters) -> Token.Value in
		guard
			let firstParameter = parameters.first,
			case let .string(boundary) = firstParameter,
			case let .string(inputString) = input
		else {
			return .nil
		}
	...
	static let strip = Filter(identifier: "strip") { (input, _) -> Token.Value in
		guard case let .string(inputString) = input else {
			return .nil
		}
	...
	static let stripHTML = Filter(identifier: "strip_html") { (input, _) -> Token.Value in
		let htmlRegex = "<[^>]+>"
		return .string(input.stringValue.replacingOccurrences(of: htmlRegex, with: "", options: .regularExpression))
	}

	static let stripNewlines = Filter(identifier: "strip_newlines") { (input, _) -> Token.Value in
		return .string(input.stringValue.replacingOccurrences(of: "\r\n", with: "")
			.replacingOccurrences(of: "\n", with: ""))
	}

	static let times = Filter(identifier: "times") { (input, parameters) -> Token.Value in
		guard let decimalInput = input.decimalValue, let decimalParameter = parameters.first?.decimalValue else {
			return .nil
		}
	...
	static let truncate = Filter(identifier: "truncate") { (input, parameters) -> Token.Value in
	...
	static let truncateWords = Filter(identifier: "truncatewords") { (input, parameters) -> Token.Value in
		...
		_ = inputString.enumerateSubstrings(in: inputString.startIndex..., options: [.localized, .byWords]) { word, range, _, stop in
	...
	static let uniq = Filter(identifier: "uniq") { (input, _) -> Token.Value in
		guard case let .array(inputArray) = input else {
			return .nil
		}
	...
	static let upcase = Filter(identifier: "upcase") { (input, _) -> Token.Value in
		return .string(input.stringValue.uppercased())
	}

	static let urlDecode = Filter(identifier: "url_decode") { (input, _) -> Token.Value in
	...
	static let urlEncode = Filter(identifier: "url_encode") { (input, _) -> Token.Value in

...
extension Operator {
	static let equals = Operator(identifier: "==") {
		.bool($0 == $1)
	}
	...
	static let contains = Operator(identifier: "contains") { lhs, rhs in
		if case let .array(array) = lhs, case let .string(string) = rhs {
			return .bool(array.contains(.string(string)))
		} else if case let .string(haystack) = lhs, case let .string(needle) = rhs {
